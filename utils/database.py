...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS weather_history (
        ...
        month_num INTEGER NOT NULL, -- 1 = Jan, 12 = Dec
        month_name TEXT NOT NULL,   -- 'January', 'February', etc.
        avg_temp REAL NOT NULL,      -- in °C
        rainfall REAL NOT NULL,      -- in mm
        description TEXT NOT NULL,
        recommendation TEXT NOT NULL,
        FOREIGN KEY (city_id) REFERENCES cities (id) ON DELETE CASCADE,
        UNIQUE(city_id, month_num)
    );
    """)
...
def get_weather_history(city_id):
    """Fetches all 12 months of weather data for a city, joined with city name, ordered calendar-wise."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        """
        SELECT weather_history.*, cities.city_name 
        FROM weather_history 
        JOIN cities ON weather_history.city_id = cities.id 
        WHERE weather_history.city_id = ? 
        ORDER BY weather_history.month_num ASC;
    """,
        (city_id,),
    )
    rows = cursor.fetchall()
...
def toggle_history_favorite(history_id, user_id):
    ...
    if row:
        ...
        cursor.execute(
            "UPDATE travel_history SET is_favorite = ? WHERE id = ? AND user_id = ?;", (new_val, history_id, user_id)
        )
        conn.commit()
    conn.close()
...
def get_dashboard_stats(user_id):
    ...
    cursor.execute(
        "SELECT COUNT(*) as count FROM saved_trips WHERE user_id = ? AND trip_type = 'itinerary';", (user_id,)
    )
    total_saved_itineraries = cursor.fetchone()["count"]

    cursor.execute(
        "SELECT COUNT(*) as count FROM travel_history WHERE user_id = ? AND activity_type = 'itinerary';", (user_id,)
    )
    total_generated_itineraries = cursor.fetchone()["count"]

    # 2. Countries Explored (distinct country keyword from history and saved)
    cursor.execute(
        "SELECT DISTINCT query FROM travel_history WHERE user_id = ? AND activity_type IN ('search', 'itinerary');",
        (user_id,),
    )
    ...
    cursor.execute(
        "SELECT COUNT(*) as count FROM saved_trips WHERE user_id = ? AND trip_type = 'destination';", (user_id,)
    )
    saved_destinations = cursor.fetchone()["count"]

    # 4. Recent Searches (top 5)
    cursor.execute(
        "SELECT DISTINCT query, created_at FROM travel_history WHERE user_id = ? AND activity_type = 'search' ORDER BY created_at DESC LIMIT 5;",
        (user_id,),
    )
    ...
    cursor.execute(
        "SELECT COUNT(*) as count FROM travel_history WHERE user_id = ? AND activity_type = 'chat';", (user_id,)
    )
    ...
    return {
        ...
        "ai_usage": {"itineraries_generated": total_generated_itineraries, "chatbot_interactions": chat_interactions},
