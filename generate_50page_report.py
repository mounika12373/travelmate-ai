...
OUTPUT_PATH = "TravelMate_AI_50Page_Final_Report.docx"
...
def add_custom_heading(doc, text, level, space_before=14, space_after=8):
...
def add_custom_paragraph(doc, text, space_after=8, line_spacing=1.25, bold=False, italic=False):
...
def add_code_listing(doc, title, file_path):
    """Reads a file and appends it to the document as a styled code block with page breaks."""
    add_custom_heading(doc, title, level=2, space_before=16, space_after=8)
    
    if not os.path.exists(file_path):
        add_custom_paragraph(doc, f"[Source file not found at: {file_path}]", italic=True)
        return
        
    with open(file_path, "r", encoding="utf-8") as f:
        code_lines = f.readlines()
        
    # Group code lines in chunks to prevent creating too many paragraph objects (improves Word performance)
    chunk_size = 50
...
def main():
    ...
    p_title_space.paragraph_format.space_before = Pt(48)
    ...
    p_subtitle.paragraph_format.space_before = Pt(24)
    ...
    if os.path.exists("assets/travelmate_banner.png"):
        ...
        p_banner.paragraph_format.space_before = Pt(24)
        p_banner.paragraph_format.space_after = Pt(24)
    ...
    r_super_desig.font.size = Pt(11.5)
    ...
    toc_data = [
        ...
        ("   1.2 Overview of the Travel Guidance & Companion Domain", "2"),
        ("   1.3 Introduction to AI and Smart Assistants in Travel", "4"),
        ("   1.4 Objectives and Scope of the Project", "5"),
        ("   1.5 System Requirements (Software & Hardware)", "7"),
        ("CHAPTER 2: LITERATURE SURVEY", "9"),
        ("   2.1 Existing Travel Planning Methods (Existing System)", "9"),
        ("   2.2 Digital Travel Platforms and Gaps (Comparison)", "10"),
        ("   2.3 Proposed System & Advantages", "12"),
        ("   2.4 Research Gap", "13"),
        ("CHAPTER 3: SYSTEM DESIGN AND ARCHITECTURE", "15"),
        ("   3.1 Project Workflow and System Overview", "15"),
        ("   3.2 System Architecture (Three-Layer Design)", "16"),
        ("   3.3 Data Flow Diagram / Flowchart", "18"),
        ("   3.4 Folder Structure and Modules Description", "19"),
        ("CHAPTER 4: DATABASE DESIGN & CORE IMPLEMENTATION", "22"),
        ("   4.1 Relational Database Schema Design", "22"),
        ("   4.2 Detailed Database Table Structures", "23"),
        ("   4.3 User Authentication & JWT Session Security", "27"),
        ("   4.4 Itinerary Planner & Cost Calculator Algorithms", "29"),
        ("CHAPTER 5: WEB APPLICATION INTERFACE", "31"),
        ("   5.1 Overview of the Streamlit Framework", "31"),
        ("   5.2 Frontend Design and User Inputs", "32"),
        ("   5.3 Page Navigation and Routing (app.py)", "33"),
        ("   5.4 Sample Prediction Walkthrough (Screenshots)", "34"),
        ("CHAPTER 6: CHALLENGES, TESTING, AND QUALITY ASSURANCE", "40"),
        ("   6.1 Technical and Practical Challenges", "40"),
        ("   6.2 BDD-style Testing Suite (Pytest)", "42"),
        ("   6.3 Code Quality, Linting, & Security Enforcement", "44"),
        ("   6.4 Results and System Evaluation", "46"),
        ("CHAPTER 7: CONCLUSION AND FUTURE SCOPE", "48"),
        ("   7.1 Summary of the Project", "48"),
        ("   7.2 Real-World Applications and Benefits", "49"),
        ("   7.3 Scope for Improvement and Extensions", "50"),
        ("REFERENCES", "52"),
        ("APPENDICES", "53"),
        ("   Appendix A: Detailed Source Code Listings", "53"),
        ("   Appendix B: Detailed SQL Schema Definitions", "73"),
        ("   Appendix C: Test Execution & Spec Verifications", "78"),
    ]
    ...
    fig_data = [
        ("Figure 3.1", "System Workflow and Logic Sequence Flowchart", "18"),
        ("Figure 5.1", "Home Dashboard Landing Page and Search Panel", "34"),
        ("Figure 5.2", "Country Information Guide Explorer (Japan Profile)", "35"),
        ("Figure 5.3", "City Explorer Interface displaying Rated Attractions (Tokyo)", "36"),
        ("Figure 5.4", "Smart Itinerary Planner input interface (3 Days, Economy)", "37"),
        ("Figure 5.5", "Plotly Budget Pie Chart and Day-by-Day Timeline output", "38"),
        ("Figure 5.6", "Detailed Budget Allocation Table and Airport Transit Tips", "38"),
        ("Figure 5.7", "AI Travel Assistant Chatbot interface (Offline Rule-Based mode)", "39"),
    ]
    ...
    tab_data = [
        ("Table 2.1", "Comparison of Proposed TravelMate AI with Existing Travel Platforms", "11"),
        ("Table 3.1", "Three-Layer Architectural Stack of TravelMate AI", "16"),
        ("Table 4.1", "Schema Structure for countries Table", "23"),
        ("Table 4.2", "Schema Structure for cities Table", "24"),
        ("Table 4.3", "Schema Structure for users Table", "25"),
        ("Table 4.4", "Schema Structure for travel_history Table", "26"),
        ("Table 4.5", "Schema Structure for saved_trips Table", "26"),
        ("Table 4.6", "Schema Structure for weather_history Table", "27"),
        ("Table 6.1", "Automated Pytest Spec-Style Verification Coverage Matrix", "43"),
    ]
    ...
    p_intro_11 = (
        "Swecha is a premier non-profit organization, operating as a regional chapter of the Free Software Movement of India "
        "(FSMI) in the states of Telangana and Andhra Pradesh. Built upon the principles of digital liberty, community empowerment, "
        "and technological autonomy, Swecha aims to make technology accessible to all sections of society. The organization plays "
        "a key role in the educational sector by training thousands of engineering students in free software methodologies. "
        "Swecha's summer and winter internship programs are designed to provide engineering undergraduates with practical, hands-on exposure "
        "to industrial software practices, including behavior-driven development (BDD) testing, containerized microservice architectures, "
        "pre-commit quality enforcement, and GitLab-based CI/CD automated deployment pipelines.\n\n"
        "By focusing on the developer's responsibility towards building clean, documented, and secure software, Swecha bridges the "
        "gap between academic theories and industry expectations. This internship project was developed under the direct guidance of "
        "technical mentors from the Swecha developer community. The project focuses on utilizing open-source frameworks to solve "
        "practical problems, ensuring that all developed modules adhere to free software licenses (like the GNU Affero General Public "
        "License) and remain accessible to the community. TravelMate AI represents a local-first application aligned with Swecha's vision."
    )
    ...
    p_intro_12 = (
        "The global tourism industry has experienced significant growth over the last decade, transitioning from travel agency bookings "
        "to dynamic, user-driven digital planning. However, this transition has introduced a new challenge for modern travelers: "
        "information fragmentation. When planning a trip to a domestic or international destination, travelers are forced to compile "
        "crucial data from dozens of scattered, non-standard sources. General information, capital locations, official languages, and currency "
        "details are searched on standard search engines. Pre-departure checklists and visa entry regulations are verified on "
        "governmental portals. Emergency services, local law warnings, and neighborhood safety guidelines require checking specialized forums. "
        "Lastly, culinary directories and lodging choices are sourced from commercial directories.\n\n"
        "This manual compilation is not only time-consuming but highly error-prone. A lack of awareness regarding local regulations—such as "
        "strict public bans and heavy fines in Singapore or specific pedestrian rules in Japan—can lead to legal liabilities and safety "
        "hazards. For budget-conscious travelers, calculating trip costs dynamically and organizing daily timelines is complex. "
        "The travel companion domain requires unified digital frameworks that consolidate regulatory country profiles, localized city explorer "
        "indices, dynamic trip schedulers, and multilingual chat assistants into a single responsive interface."
    )
    ...
    p_intro_13 = (
        "Conversational artificial intelligence (AI) has revolutionized human-computer interaction, transitioning applications from rigid, "
        "menu-driven systems to flexible natural language interfaces. In the travel domain, intelligent conversational agents—commonly "
        "referred to as smart travel assistants—allow travelers to ask questions and receive structured answers instantly. "
        "A critical requirement for travel assistants is context awareness. When a traveler on the go asks a question like "
        "'What food should I try?' or 'Where should I stay?', a standard chatbot requires them to specify the destination. "
        "A context-aware travel assistant, however, automatically resolves ambiguity by retrieving the active country or city "
        "selected in the user interface session state, ensuring a seamless user experience.\n\n"
        "To deliver maximum utility, travel chatbots must incorporate two operational modes: (1) Autonomous Agent Mode, utilizing modern "
        "agent frameworks (like Google Agent Development Kit) and large language models (like Gemini) to dynamically call database search "
        "tools; and (2) Offline Rule-Based Mode, utilizing local keyword tokenization and regex matching to query local SQLite tables "
        "without requiring active internet connectivity. This hybrid architecture ensures that travelers can access critical safety, cultural, "
        "and geographical insights even under restricted network conditions, making smart assistants highly reliable companions."
    )
    ...
    p_intro_14 = (
        "The core objective of this project is to design, develop, test, and deploy a production-grade, open-source travel guidance "
        "and companion web application named TravelMate AI. The detailed objectives include:\n"
    )
    add_custom_paragraph(doc, p_intro_14)
    add_bullet_point(doc, "1. Centralized Travel Knowledge Base: ", "Develop and populate a relational SQLite database structure that compiles comprehensive visa checklists, emergency contacts, local rules, cultural etiquette, attractions, dining options, and accommodation tiers for major destinations.")
    add_bullet_point(doc, "2. Multilingual User Accessibility: ", "Implement a complete internationalization (i18n) model supporting English, Hindi, and Telugu equivalents for all UI elements, database entries, and chatbot translation mappings to serve diverse traveler groups.")
    add_bullet_point(doc, "3. Dynamic Itinerary Generation & Cost Allocation: ", "Design an automated trip scheduling algorithm that generates customized, day-by-day timelines for 1-7 days based on selected cities and budget levels, visualized using interactive Plotly Express pie charts.")
    add_bullet_point(doc, "4. Context-Aware Hybrid Chatbot Assistant: ", "Construct a conversational assistant that supports autonomous tool-based SQL queries using Google ADK and Gemini, alongside a local keyword fallback parsing algorithm linked to UI session state context.")
    add_bullet_point(doc, "5. User Session Security & History Logging: ", "Enforce secure user access using SHA-256 password hashing and HMAC-SHA256 JWT tokens, with database persistence for travel search history, saved itineraries, and user profile analytics.")
    add_bullet_point(doc, "6. Rigid Quality Assurance & Continuous Deployment: ", "Enforce software engineering best practices by writing a comprehensive Pytest BDD suite, integrating git pre-commit quality hooks, containerizing the app via Docker, and configuring GitLab CI/CD pipelines.")
    
    add_custom_heading(doc, "1.5 System Requirements (Software & Hardware)", level=2, space_before=12, space_after=6)
    p_intro_15 = (
        "To ensure successful compilation, testing, and deployment of the TravelMate AI platform, the development and execution "
        "environments must meet the following hardware and software requirements:\n"
    )
    add_custom_paragraph(doc, p_intro_15)
    add_bullet_point(doc, "• Hardware Requirements (Minimum Specifications): ", "Intel Core i5 or AMD Ryzen 5 processor (4 physical cores, 2.5 GHz+ clock speed); 8 GB DDR4 System RAM; 5 GB SSD available storage; and stable network connectivity for API key validation.")
    add_bullet_point(doc, "• Software Requirements (Development Environment): ", "Windows 10/11 64-bit OS or Ubuntu Linux 20.04+ LTS OS; Python Runtime environment (v3.9 or higher); and SQLite 3 database engine.")
    add_bullet_point(doc, "• Primary Python Library Dependencies: ", "Streamlit (v1.35.0+), Pandas (v2.0.0+), Plotly Express (v5.15.0+), Pytest (v8.0.0+), and google-adk (v0.1.0+).")
    add_bullet_point(doc, "• Code Quality & Security Tools: ", "Ruff (linter and formatter), Mypy (static type checker), Bandit (security scanner), Gitleaks (secret protection), and pip-audit (vulnerability auditor).")
    ...
    p_lit_21 = (
        "Traditional travel planning is fundamentally manual and decentralized. Before embarking on a journey, travelers must "
        "spend hours compiling destination information. Official requirements like visas and vaccinations are verified on "
        "government websites. Safety warnings and emergency contacts require reading through extensive text advisories. "
        "Recreational details, sightseeing spots, and lodging listings are compiled from commercial blogs. "
        "This manual process is inefficient and prone to oversight. Travelers frequently miss local regulations or cultural "
        "etiquette guidelines, leading to legal penalties or social discomfort. Furthermore, static travel guides lack interactive "
        "planning tools, and the absence of context-aware digital assistants leaves users to filter information manually."
    )
    ...
    p_lit_22 = (
        "Numerous digital platforms exist to assist travelers, yet a critical analysis reveals major feature gaps. Commercial platforms "
        "(such as TripAdvisor, Booking.com, and Expedia) focus heavily on transaction-driven features (booking hotel stays, flights, "
        "and activities) or crowdsourced reviews. However, they lack structured legal checklists, regional regulatory warnings, "
        "and local emergency contact directories. Government advisories offer comprehensive security details but are text-heavy, "
        "lack city-level transit and attraction guides, and do not provide interactive itinerary generators.\n"
    )
    add_custom_paragraph(doc, p_lit_22)
    
    # Table 2.1: Gaps Comparison
    p_tab_cap21 = doc.add_paragraph()
    r_tab_cap21 = p_tab_cap21.add_run("Table 2.1: Comparison of Proposed TravelMate AI with Existing Travel Platforms")
    r_tab_cap21.font.size = Pt(10)
    r_tab_cap21.bold = True
    ...
    p_lit_23 = (
        "The proposed TravelMate AI system resolves these limitations by offering a unified, secure, and multilingual travel companion "
        "platform. The application combines regulatory and cultural safety guides with rated attractions, dining, and hotels in a single dashboard. "
        "Key advantages of TravelMate AI include: (1) Centralized SQL travel data; (2) Dynamic day-by-day itineraries (1-7 days) "
        "with Plotly budget breakdowns; (3) A context-aware chatbot using Google ADK Gemini and local keyword fallbacks; "
        "(4) Complete localization in English, Hindi, and Telugu; and (5) Secure JWT session persistence and SHA-256 password hashing. "
        "The application is fully containerized via Docker and deployed on Streamlit Community Cloud."
    )
    add_custom_paragraph(doc, p_lit_23)
    
    add_custom_heading(doc, "2.4 Research Gap", level=2, space_before=12, space_after=6)
    p_lit_24 = (
        "An analysis of literature in the digital tourism domain highlights a major research gap: the lack of integration between "
        "commercial tourism listings and regulatory/cultural safety guidelines. Most existing systems separate travel planning from "
        "legal compliance. Furthermore, existing conversational travel assistants are typically general-purpose chatbots that lack "
        "integration with local travel databases and do not inherit the user's active page navigation as context. Finally, "
        "multilingual localization supporting regional Indian languages (English, Hindi, and Telugu) is rarely implemented in travel portals. "
        "TravelMate AI directly addresses these gaps by implementing a context-aware hybrid chatbot and unified localized data structures."
    )
    ...
    p_arch_31 = (
        "TravelMate AI follows a modular project workflow designed to move requests efficiently between user UI pages, "
        "database connection helpers, and translation dictionaries. On startup, app.py initializes the SQLite database schema and "
        "seeds initial sample data for India, Japan, and Singapore if empty. Active country and city selection states are saved "
        "in the Streamlit session state, setting the global context across all pages. Changes in the language selection dropdown "
        "instantly translate the user interface, while the search bar queries multiple columns across both countries and cities tables."
    )
    ...
    p_arch_32_desc = (
        "The platform implements a clean, decoupled three-layer architecture to enforce separation of concerns, "
        "independent scaling, and high maintainability across the Streamlit-based deployment environment:\n"
    )
    add_custom_paragraph(doc, p_arch_32_desc)
    
    # Table 3.1: Layers
    p_tab_cap31 = doc.add_paragraph()
    r_tab_cap31 = p_tab_cap31.add_run("Table 3.1: Three-Layer Architectural Stack of TravelMate AI")
    r_tab_cap31.font.size = Pt(10)
    r_tab_cap31.bold = True
    ...
    p_arch_33_intro = "The flowchart below describes the operational data and logic flow from initialization to screen rendering:"
    ...
    p_arch_34_intro = "The codebase is organized in a modular structure to enforce code separation and facilitate FOSS development:"
    add_custom_paragraph(doc, p_arch_34_intro)
    
    add_bullet_point(doc, "• app.py: ", "The application's entry point. Configures page titles, icons, Layout settings, and handles language selector logic and sidebar styling injections.")
    add_bullet_point(doc, "• pages/home.py: ", "The landing view of the website. Implements the featured destination cards, search bar, and handles routing based on search parameters.")
    add_bullet_point(doc, "• pages/country_info.py: ", "Renders country-specific summaries, visa checklists, and cultural rules.")
    add_bullet_point(doc, "• pages/city_info.py: ", "Displays rated sightseeing spots, local dining recommendations, hotel price lists, and transit details.")
    add_bullet_point(doc, "• pages/planner.py: ", "Captures custom duration and budget inputs, generates day-by-day itineraries, and renders Plotly Express cost charts.")
    add_bullet_point(doc, "• pages/chatbot.py: ", "Maintains the conversational user interface. Displays predefined chips and handles text submission to the ADK agent or local keyword fallback.")
    add_bullet_point(doc, "• pages/auth.py, profile.py, history.py, saved_trips.py: ", "Manage registration, JWT login, profile picture base64 storage, user search history logs, saved itineraries, and dashboard statistics.")
    add_bullet_point(doc, "• utils/database.py: ", "Handles relational database connections, schema creation, query helpers, and seeding.")
    add_bullet_point(doc, "• utils/auth_utils.py: ", "Implements SHA-256 password hashing and JWT encoding/decoding signed with HMAC-SHA256.")
    add_bullet_point(doc, "• utils/styles.py: ", "Contains global CSS stylesheet overrides, glassmorphism cards, and Outfit google font injection scripts.")
    add_bullet_point(doc, "• utils/i18n.py: ", "Contains the multilingual translation dictionary and query normalization helpers.")
    ...
    style_table_row(table_pytest.rows[2], ["test_auth_spec.py", "User registration, login, and JWT security", "Verifies SHA-256 hashes, encodes/decodes JWT tokens, checks token expiry/tampering"], is_even=True)
    ...
    p_qa_63 = (
        ...
        "(3) Security Scanning: Bandit detects code security flaws (hardcoded credentials, unsafe calls), and Gitleaks scans commits for secrets; "
    ...
    add_bullet_point(doc, "• LLM Integration: ", "Integrate Large Language Models (LLMs) such as GPT-4 or Gemini to enable semantic, generative chatbot responses beyond rule-based keyword matching.")
    ...
    # ==========================================
    # APPENDICES (Fleshing out code listings to expand report to 50 pages)
    # ==========================================
    add_custom_heading(doc, "Appendices", level=1, space_before=24, space_after=18)
    
    # Appendix A - Detailed Code Listings
    add_custom_heading(doc, "Appendix A: Detailed Source Code Listings", level=2, space_before=12, space_after=6)
    p_code_intro = (
        "Following standard B.Tech academic formatting guidelines, this appendix lists the full source code files for the "
        "core modules of the TravelMate AI platform. These listings are presented in Courier New monospaced font "
        "and contain full comments to explain code flow, SQL queries execution, JWT security signature, and i18n localization dictionaries."
    )
    add_custom_paragraph(doc, p_code_intro)
    
    add_code_listing(doc, "A.1 Database Connection and SQL Helper Functions (utils/database.py)", "utils/database.py")
    doc.add_page_break()
    
    add_code_listing(doc, "A.2 Cryptography and JWT Authentication Handler (utils/auth_utils.py)", "utils/auth_utils.py")
    doc.add_page_break()
    
    add_code_listing(doc, "A.3 Autonomous AI Agent and Tool Invoker (utils/chatbot_agent.py)", "utils/chatbot_agent.py")
    doc.add_page_break()
    
    add_code_listing(doc, "A.4 User Interface View for AI Travel Assistant (pages/chatbot.py)", "pages/chatbot.py")
    doc.add_page_break()
    
    # Appendix B - SQL Schemas
    add_custom_heading(doc, "Appendix B: Detailed SQL Schema Definitions", level=2, space_before=12, space_after=6)
    p_app_b_text = (
        "The following represents the complete SQL schema DDL scripts executed by init_db() to construct the relational tables:\n\n"
    ...
    p_sql_ddl = doc.add_paragraph()
    run_sql_ddl = p_sql_ddl.add_run(p_app_b_text)
    run_sql_ddl.font.name = 'Courier New'
    run_sql_ddl.font.size = Pt(9)
    run_sql_ddl.font.color.rgb = RGBColor(27, 54, 93)
    
    doc.add_page_break()
    
    # Appendix C - BDD Spec
    add_custom_heading(doc, "Appendix C: Test Execution & Spec Verifications", level=2, space_before=12, space_after=6)
    p_app_c_text = (
        "The following BDD spec-style verification scenarios are executed via pytest tests/ to validate chatbot responses and "
        "context engine fallback logic:\n\n"
    ...
    p_test_bdd = doc.add_paragraph()
    run_test_bdd = p_test_bdd.add_run(p_app_c_text)
    run_test_bdd.font.name = 'Courier New'
    run_test_bdd.font.size = Pt(9)
    run_test_bdd.font.color.rgb = RGBColor(27, 54, 93)
    
    doc.save(OUTPUT_PATH)
    print(f"50-page Final Project Report successfully generated and saved to: {OUTPUT_PATH}")
