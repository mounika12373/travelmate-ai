...
from utils.database import get_saved_trips, delete_saved_trip, save_trip
...
render_hero("💾 Saved Trips & Collections", "Manage your saved itineraries, bookmarked destinations, and custom travel collections")
        ...
        for idx, item in enumerate(itineraries):
            ...
            with st.container(border=True):
                ...
                with col_header1:
                    ...
                    st.caption(f"Planned Date: **{item['travel_date'] or 'Not Set'}** | Saved on {item['created_at'][:10]}")
                with col_header2:
...
with tab_destinations:
    ...
    if not destinations:
        st.info(f"No bookmarked destinations in '{selected_collection}'. Search and bookmark them from the Home dashboard!")
    else:
        ...
        for item in flights:
            ...
            with st.container(border=True):
                ...
                with col_f1:
                    ...
                    st.write(f"Departure: **{details.get('departure_time', 'N/A')}** | Arrival: **{details.get('arrival_time', 'N/A')}**")
