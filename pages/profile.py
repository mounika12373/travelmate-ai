...
from utils.database import get_dashboard_stats, get_user_by_id, update_user_profile
    ...
    if upcoming:
        for trip in upcoming:
            try:
                ...
                details = {}
            st.info(
                f"✈️ **{trip['name']}** - Planned Date: **{trip['travel_date']}** | Budget Level: **{details.get('budget_tier', 'N/A')}**"
            )
    else:
        st.info(
            "No upcoming trips found. Create a plan in the [Travel Planner](pages/planner.py) page and save it with a travel date!"
        )
